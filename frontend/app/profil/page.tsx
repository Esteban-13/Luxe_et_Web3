...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    setEmail(user)
    const addr = localStorage.getItem("timury_smart_address")
    if (addr) setBuyerAddress(addr)
  }, [router])

  const myCertificats = watches.filter(
    (w) => buyerAddress && w.owner.toLowerCase() === buyerAddress.toLowerCase()
  )

  function handleLogout() {
    localStorage.removeItem("timury_user")
    localStorage.removeItem("timury_cart")
    localStorage.removeItem("timury_smart_address")
    router.push("/")
  }
